...
function extractApiError(payload: unknown, fallback: string) {
  ...
  const data = payload as {
    detail?: string | { message?: string; code?: string } | Array<{ msg?: string; msg_detail?: string; message?: string }>;
    message?: string;
    code?: string;
  };
  ...
  if (Array.isArray(data.detail) && data.detail.length) {
    const messages = data.detail
      .map((item) => item.message || item.msg_detail || item.msg)
      .filter((item): item is string => Boolean(item));
    if (messages.length) {
      return { message: messages.join('. '), code: data.code };
    }
  }
