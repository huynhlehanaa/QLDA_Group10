...
export function useUsers() {
  ...
  const resetManagerPassword = useCallback((id: string) => run(() => (
    apiRequest<{ message: string }>(`/api/v1/users/managers/${id}/reset-password`, { method: 'POST', token: accessToken })
  )), [accessToken, run]);
  ...
  const resetStaffPassword = useCallback((id: string) => run(() => (
    apiRequest<{ message: string }>(`/api/v1/users/staff/${id}/reset-password`, { method: 'POST', token: accessToken })
  )), [accessToken, run]);
