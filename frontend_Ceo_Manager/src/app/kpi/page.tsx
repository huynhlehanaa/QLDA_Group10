    ...
    <main style={{ maxWidth: 1200, margin: '0 auto', padding: 24, display: 'grid', gap: 16 }}>
      ...
      {(me?.role === 'manager' || me?.role === 'ceo') ? (
        <section style={cardStyle}>
          ...
          {/* 🆕 NÂNG CẤP BIỂU ĐỒ TRÒN/DONUT PHÂN PHỐI XẾP LOẠI KPI PHÒNG BAN (PB145) */}
          <div style={{ background: '#f8fafc', padding: 16, borderRadius: 8, border: '1px solid #e2e8f0', margin: '8px 0', display: 'flex', alignItems: 'center', gap: 40, flexWrap: 'wrap' }}>
            {(() => {
              ...
              // Tính toán phân bổ hình tròn SVG
          ...
          {/* 🆕 NÂNG CẤP BẢNG XẾP HẠNG DANH HIỆU KPI ĐÍNH HUY CHƯƠNG (PB143) */}
          <div style={{ marginTop: 14 }}>
            <h3 style={{ fontSize: 15, fontWeight: 600, color: '#1e293b', margin: '0 0 10px' }}>Top 5 Nhân Sự Xuất Sắc Nhất 🥇</h3>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ textAlign: 'left', background: '#f8fafc' }}>
                    ...
                    <th style={{ padding: 8 }}>Nhân viên</th>
                    <th style={{ padding: 8 }}>Điểm số đạt được</th>
                  </tr>
                </thead>
                <tbody>
                  {deptRanking.slice(0, 5).map((item, index) => {
                    ...
                    return (
                      <tr key={item.user_id} style={{ borderTop: '1px solid #e2e8f0', background: index < 3 ? 'rgba(245, 158, 11, 0.03)' : 'transparent' }}>
      ...
      {me?.role === 'ceo' ? (
        <section style={cardStyle}>
          <h2 style={{ margin: 0, fontSize: 18 }}>Phêu duyệt điều chỉnh KPI (CEO)</h2>
