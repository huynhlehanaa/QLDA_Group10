...
export default function TasksPage() {
  ...
  // State hỗ trợ tính năng xem chi tiết Drawer mới (PB083)
  ...
  }, [accessToken, fetchKanban, fetchStats, fetchTasks, priorityFilter, search, statusFilter, selectedTask]);
  ...
  return (
    <main style={{ maxWidth: 1200, margin: '0 auto', padding: 24, display: 'grid', gap: 16 }}>
            ...
            <tbody>
              {tasks.map((task) => (
                <tr 
                  ...
                  onClick={() => setSelectedTask(task)} // Kích hoạt sự kiện click mở Drawer (PB083)
                >
                  <td style={{ padding: 8 }}>
      ...
      {/* TÍCH HỢP ĐỒNG BỘ DRAWER CHI TIẾT TASK NÂNG CAO (PB083, PB071, PB091, PB092) */}
      {selectedTask && (
...
// ─── TÁCH CÁC COMPONENT BỔ SUNG THEO MẪU ĐỂ CHÈN VÀO DRAWER ──────────────────

function TaskDetailDrawerExtended({ task, currentUser, onClose, onChangeStatus }: any) {
  const [activeSection, setActiveSection] = useState('info');
  
  // Fake state cho các tính năng checklist & bình luận hoạt động mượt mà ở Frontend
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 300, overflowY: 'auto' }}>
                {comments.map(c => (
                  <div key={c.id} style={{ background: '#f8fafc', borderRadius: 6, padding: '8px 10px', border: '1px solid #f1f5f9' }}>
                    ...
                    {/* Luồng phản hồi Reply Thread (PB091) */}
                    {c.replies.map((r: any) => (
