...
export default function KpiPage() {
  ...
  async function onCreateAppeal(event: FormEvent<HTMLFormElement>) {
    ...
    setError(''); setNotice('');
    try {
      const result = await createAppeal({ year, month, criteria_name: appealCriteria, current_score: Number(appealCurrentScore), proposed_score: Number(appealProposedScore), reason: appealReason });
  ...
  async function onCreateAdjustment(event: FormEvent<HTMLFormElement>) {
    ...
    setError(''); setNotice('');
    try {
      const result = await createAdjustment({ user_id: adjustUserId, year, month, criteria_name: adjustCriteria, proposed_score: Number(adjustProposedScore), reason: adjustReason });
  ...
  async function onRespondAppeal(event: FormEvent<HTMLFormElement>) {
    ...
    setError(''); setNotice('');
    try {
      await respondAppeal(appealIdToRespond, { approved: appealApprove === 'approve', response: appealResponseText, adjusted_score: appealApprove === 'approve' && appealAdjustedScore !== '' ? Number(appealAdjustedScore) : undefined });
  ...
  async function onReviewAdjustment(event: FormEvent<HTMLFormElement>) {
    ...
    setError(''); setNotice('');
    try {
      await reviewAdjustment(reviewAdjId, { approved: reviewApprove === 'approve', comment: reviewComment || undefined });
        ...
        <nav className="navList">
          {visibleTabs.map((tab) => (
            <button
              key={tab.key}
              type="button"
              className={tab.key === 'kpi' ? 'navItem active' : 'navItem'}
        ...
        <div className="gridStack">
          <section style={cardStyle}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button type="button" className="primaryButton" onClick={() => loadKpiData()}>{loading ? 'Đang tải...' : 'Tải dữ liệu'}</button>
            </div>
          </section>
          ...
          <section style={cardStyle}>
            ...
            <form onSubmit={onSetTarget} style={{ display: 'flex', gap: 8 }}>
              ...
              <button type="submit" className="primaryButton" disabled={loading}>{loading ? 'Đang lưu...' : 'Lưu mục tiêu'}</button>
            </form>
          </section>
          ...
          <section style={cardStyle}>
            ...
            <form onSubmit={onCreateAppeal} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 2fr auto', gap: 8 }}>
              ...
              <button type="submit" className="primaryButton" disabled={loading}>Gửi</button>
            </form>
          </section>
          ...
          {(me?.role === 'manager' || me?.role === 'ceo') ? (
            <section style={cardStyle}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button type="button" className="secondaryButton" onClick={onExportDept} disabled={loading}>Xuất Excel phòng ban</button>
              </div>
            </section>
          ) : null}
          ...
          {me?.role === 'manager' ? (
            <section style={cardStyle}>
              ...
              <form onSubmit={onCreateAdjustment} style={{ display: 'grid', gridTemplateColumns: '1.2fr 1.2fr 1fr 2fr auto', gap: 8 }}>
                ...
                <button type="submit" className="primaryButton" disabled={loading || !adjustUserId}>Gửi</button>
              </form>
            </section>
          ) : null}
          ...
          {me?.role === 'ceo' ? (
            <section style={cardStyle}>
              ...
              <form onSubmit={onReviewAdjustment} style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 2fr auto', gap: 8 }}>
                ...
                <button type="submit" className="primaryButton" disabled={loading}>Xử lý</button>
              </form>
            </section>
          ) : null}
          ...
          {me?.role === 'ceo' ? (
            <section style={cardStyle}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button type="button" className="primaryButton" onClick={onFinalizeKpi} disabled={loading}>Chốt KPI tháng</button>
                <button type="button" className="secondaryButton" onClick={() => onExportCompany(true)} disabled={loading}>Xuất Excel công ty (tháng)</button>
                <button type="button" className="secondaryButton" onClick={() => onExportCompany(false)} disabled={loading}>Xuất Excel công ty (năm)</button>
              </div>
              <form onSubmit={onUnlockKpi} style={{ display: 'grid', gridTemplateColumns: '2fr auto', gap: 8 }}>
                ...
                <button type="submit" className="primaryButton" disabled={loading}>Mở khóa KPI</button>
              </form>
            </section>
          ) : null}
        </div>
